...
function show(screenId) {
    ...
    if (!el)
        throw new Error(`No existe pantalla: ${screenId}`);
...
function initCaptcha1() {
    ...
    // circle
    ctx2.fillStyle = "#3fb4ff";
    ctx2.beginPath();
    ctx2.arc(circle.x, circle.y, circle.r, 0, Math.PI * 2);
    ctx2.fill();
    ctx2.strokeStyle = "rgba(255,255,255,0.7)";
    ctx2.lineWidth = 3;
    ctx2.stroke();
    // rect
    ctx2.fillStyle = "#9dd3ff";
    ...
    // triangle
    ctx2.fillStyle = "#7ec8ff";
    ctx2.beginPath();
    ctx2.moveTo(tri.pts[0].x, tri.pts[0].y);
    ctx2.lineTo(tri.pts[1].x, tri.pts[1].y);
    ctx2.lineTo(tri.pts[2].x, tri.pts[2].y);
    ctx2.closePath();
    ctx2.fill();
    ctx2.stroke();
    ...
    function handler(ev) {
        ...
        if (dx * dx + dy * dy <= circle.r * circle.r) {
            ...
            if (hint)
                hint.textContent = "Correcto! avanzando...";
            ...
            flashCanvas(c);
            if (hint)
                hint.textContent = "Intenta de nuevo";
            // borra mensaje después
            setTimeout(() => { if (hint)
                hint.textContent = ""; }, 900);
        }
    }
    // attach handlers
    c.addEventListener("click", handler);
    c.addEventListener("touchstart", handler);
}
/* -------------------------
   Captcha 2: triangle with noise
   ------------------------- */
function initCaptcha2() {
    const canvas = document.getElementById("captcha2-canvas");
...
function initCaptcha3() {
    const canvas = document.getElementById("captcha3-canvas");
    ...
    function handler(ev) {
        ...
        let ok = false;
        if (current.id === "circle") {
            const dx = x - current.x, dy = y - current.y;
            ok = dx * dx + dy * dy <= (current.r * current.r);
        }
        else if (current.id === "square") {
            ok = x >= current.x && x <= current.x + current.w && y >= current.y && y <= current.y + current.h;
        }
...
function initCaptcha4() {
    ...
    const ok = document.getElementById("c4-ok");
...
function initCaptcha5() {
    const canvas = document.getElementById("captcha5-canvas");
    ...
    const ok = document.getElementById("c5-ok");
    ok.addEventListener("click", () => {
...
function addNoiseToCanvas(ctx, canvas, alpha = 0.08) {
...
function drawTargets3(ctx, targets) {
    // circle
    ctx.fillStyle = "#5bd0ff";
    ctx.beginPath();
    ctx.arc(targets[0].x, targets[0].y, targets[0].r, 0, Math.PI * 2);
    ctx.fill();
    // square
    ctx.fillStyle = "#85e0ff";
    ctx.fillRect(targets[1].x, targets[1].y, targets[1].w, targets[1].h);
    // diamond
    ctx.save();
...
function markSelection3(ctx, t) {
    ...
    if (t.id === "circle") {
        ...
        ctx.rotate(Math.PI / 4);
        ctx.fillRect(-t.w / 2 - 6, -t.h / 2 - 6, t.w + 12, t.h + 12);
    }
    ctx.restore();
}
/* -------------------------
...
const observer = new MutationObserver((mut) => {
    for (const m of mut) {
        if (m.type === "attributes" && m.target.classList.contains("screen")) {
            // nothing
        }
    }
});
