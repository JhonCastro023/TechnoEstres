...
const screens = {
    ...
    "final-loading": document.getElementById("final-loading-screen"),
...
const controlsEl = document.getElementById("captcha-controls");
...
function runCaptcha4() {
    ...
    const input = document.getElementById("captcha4-input");
    const okBtn = document.getElementById("captcha4-ok");
...
function runCaptcha5() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (let x = 0; x < canvas.width; x += 10)
            ctx.lineTo(x, 30 + i * 60 + Math.sin(x / 30 + i) * 12);
        ctx.strokeStyle = `rgba(10,30,60,${0.12 + i * 0.02})`;
        ctx.lineWidth = 2;
        ctx.stroke();
    }
    ...
    const input = document.getElementById("captcha5-input");
    const okBtn = document.getElementById("captcha5-ok");
    okBtn.addEventListener("click", () => {
...
/* --- Helpers --- */
function showTemporaryMessage(msg, cb) {
    hintEl.textContent = msg;
    setTimeout(() => { hintEl.textContent = ""; if (cb)
        cb(); }, 900);
}
function flashRed() {
